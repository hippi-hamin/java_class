package ch11_array.ex2;

import java.util.ArrayList;
import java.util.List;

public class StudentMain {
    public static void main(String[] args) {
        // Student 클래스 객체를 담기 위한 List
        // studentList

        List<Student> studentList = new ArrayList<>();
//        studentList.add(12); // x
        String studentName = "학생1";
//        studentList.add(studentName); // x

        // Student객체 student1 만들기
        Student student1 = new Student(1L, "학생1", 20231111, "현대무용", "010-1111-1111"); // 자바문법 Long타입의 데이터를 입력할 때 숫자 뒤에 대문자 L을 꼭 써주기
        // studentList에 student1 추가
        studentList.add(student1);
        // Student객체 student2 만들기
        Student student2 = new Student(2L, "학생2", 20232222,"전자공학","010-2222-2222");
        // studentList에 student2 추가
        studentList.add(student2);

        // studentList에 추가한 Student객체들의 정보 확인하는 법
        System.out.println("studentList.get(0) = " + studentList.get(0));
        System.out.println("studentList.get(1) = " + studentList.get(1));

        Student a = studentList.get(0); // Student 라는 타입을 갖고 있는 a = studentList.get(0)의 값
                                        // a는 Student 라는 객체가 된거임.
        System.out.println("a = " + a);
        System.out.println(a.getStudentName());

        // 1번 인덱스에 담긴 객체의 학과만 출력
        // 방법 1
        System.out.println(studentList.get(1).getStudentMajor());
        // 방법 2
        Student b = studentList.get(1);
        System.out.println(b.getStudentMajor());

        // 반복문
        for (int i = 0; i < studentList.size(); i++) {
            System.out.println("studentList = " + studentList.get(i));
            System.out.println("studentList = " + studentList.get(i).getStudentName());
        }

        for (Student stu : studentList){
            System.out.println("stu = " + stu);
            // 이름만 출력
            System.out.println("stu.getStudentName() = " + stu.getStudentName());
        }
    }
}
